...
pokemon_t *pokemon_crear(char *nombre, int puntaje, char *color,
			 char *movimientos)
{
...
bool pokedex_agregar_pokemon(pokedex_t *pokedex, char *ruta)
{
	...
	bool (*parseadores[])(const char *,
			      void *) = { parsear_string, parsear_entero,
					  parsear_string, parsear_string };
	...
	while (leer_linea_csv(archivo, 4, parseadores, ctx) == 4) {
		...
		pokemon_t *pokemon =
			pokemon_crear(nombre, puntaje, color_ansi, movimientos);
